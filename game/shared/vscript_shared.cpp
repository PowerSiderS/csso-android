...
HSCRIPT VScriptCompileScript( const char *pszScriptName, bool bWarnMissing )
{
	...
	{
		ConDWarning( "Script file type does not match VM type\n" );
		return NULL;
	}
	...
	if( !bResult )
	{
		ConDWarning( "Script not found (%s) \n", scriptPath.operator const char *() );
	...
	if ( !hScript )
	{
		ConDWarning( "FAILED to compile and execute script file named %s\n", scriptPath.operator const char *() );
		Assert( "Error running script" );
	}
	return hScript;
}
...
bool VScriptRunScript( const char *pszScriptName, HSCRIPT hScope, bool bWarnMissing )
{
	...
	{
		ConDWarning( "Cannot run script: NULL script name\n" );
		return false;
	}
	...
	{
		ConDWarning( "IncludeScript stack overflow\n" );
		return false;
	}
	...
	if ( hScript )
	{
		...
		if ( !bSuccess )
		{
			ConDWarning( "Error running script named %s\n", pszScriptName );
			Assert( "Error running script" );
		}
	}
	g_ScriptServerRunScriptDepth--;
...
#endif
{
	...
	if ( !*args[1] )
	{
		ConDWarning( "No function name specified\n" );
		return;
	}

	if ( !g_pScriptVM )
	{
		ConDWarning( "Scripting disabled or no server running\n" );
		return;
	}
...
#endif
{
	...
	if ( !*args[1] )
	{
		ConDWarning( "No script specified\n" );
		return;
	}

...
int CSosOperatorStack::GetOperatorOutputOffset( const char *pOperatorName, const char *pOutputName )
{
	...
	{
		DWarning( "SndOperators", 1, "Error: Unable to find referenced operator: %s", pOperatorName );
		return -1;
	}
...
void CSosOperatorStack::Shutdown( )
{
	...
	// 	{
	// 		DMsg( "SndOperators", 1, "Operator Stack: %s\n", m_nName );
	// 	}
...
void CSosOperatorStack::ExecuteIterator( channel_t *pChannel, CScratchPad *pScratchPad, const void *pStopMemBlock, const char *pOperatorName, int * pnOperatorIndex )
{
	...
	if ( nIndex < 0 )
	{
		...
		if ( nIndex < 0 )
		{
			DWarning( "SndOperators", 1, "Error: Execute iterator unable to find sound operator %s\n", pOperatorName );
			return;
		}
...
void CSosOperatorStack::Copy( CSosOperatorStack *pSrcStack, size_t nMemOffset )
{
	...
	for ( int i = 0; i < pSrcStack->m_vStack.Count(); i++ )
	{
		...
		{
			DWarning( "SndOperators", 1, "Error: Importing operator with same name as existing operator: %s\n", pName );
		}
			...
			if ( pParams )
			{
				...
				else if( !( testDict.IsValidIndex( nNewStackIndex ) && i == SND_SOS_MEMORY_ALLOCATION_PASS ) ) // do nothing if override of new op on first memory accumulating pass
				{
					...
					if( pOpType && *pOpType )
					{
						...
						{
							DWarning( "SndOperators", 1, "Error: Operator: %s : Unknown sound operator type %s\n", pOpName, pOpType );
...
KeyValues *S_GetStopTracksKV( KeyValues *pOperatorsKV )
{
	if( !pOperatorsKV )
	{
		...
		{
			DWarning( "SndOperators", 1, "Error: Sound Operator System has invalid operator KV\n" );
		}
		return NULL;
	}
	...
	if( ! pOperatorDataKV )
	{
		//		DWarning( "SndOperators", 1, "Error: Sound Operator System cannot find \"soundentry_operator_data\"\n" );
		return NULL;
	}
...
// 					pDefaultStack = "cue_default";
				...
				{
					DWarning( "SndOperators", 1, "Error: Unknown sound operator stack type: %s\n", pStackListTypeString );
...
CSosOperatorStackList *S_InitChannelOperators( stack_data_t &stackData )
{
	...
	if ( ! pStackList )
	{
// 		DWarning( "SndOperators", 1, "Error: Unable to create operator stack list on channel\n");
// 		return;
	}
	else
	{
...
void CSosOperatorStackCollection::ParseKV( CSosOperatorStack::SosStackType_t SosType, KeyValues *pStackType )
{
	...
	while ( pStack )
	{
		...
		default:
			// BETTER ERROR HERE
			DWarning( "SndOperators", 1, "Error: Unknown sound operator stack type" );
			delete pNewStack;
			break;
		}
...
void CSosOperatorSystem::Init()
{
	...
	//	AccumulateFileNameAndTimestampIntoChecksum( &crc, SOUND_OPERATORS_FILE );
		...
		while ( pStackType )
		{
			...
			if ( pStackTypeString && *pStackTypeString )
			{
				...
				{
					// BETTER ERROR HERE
					DWarning( "SndOperators", 1, "Unknown sound operator stack arg %s", pStackTypeString  );
					continue;
				}
			}
	...
	{
		DWarning( "SndOperators", 1, "Warning: Unable to load sound operators file '%s'\n", SOUND_OPERATORS_FILE );
	}
	...
	for( int i = m_sosStartEntryQueue.Count() -1; i > -1; i-- )
	{
		...
		if( pStartQueue->m_nStartTime <= g_pSoundServices->GetHostTime() )
		{
			...
			if( !bHasPriority && bHasData && bHasData2 )
			{
				if( snd_sos_show_startqueue.GetInt() )
				{
					DMsg( "SndOperators", 1, "STARTQUEUE: Not starting %s due to existing priority.\n", g_pSoundEmitterSystem->GetSoundNameForHash( pStartQueue->m_startSoundParams.m_nSoundScriptHash ) );
				}
			}
			else
			{
				if( snd_sos_show_startqueue.GetInt() )
				{
					DMsg( "SndOperators", 1, "STARTQUEUE: %s StartSoundEntry\n", g_pSoundEmitterSystem->GetSoundNameForHash( pStartQueue->m_startSoundParams.m_nSoundScriptHash ) );
				}

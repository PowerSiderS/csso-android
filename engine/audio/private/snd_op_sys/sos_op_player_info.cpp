...
void CSosOperatorViewInfo::Execute( void *pVoidMem, channel_t *pChannel, CScratchPad *pScratchPad, CSosOperatorStack *pStack, int nOpIndex ) const
{
	...
	{
		VectorCopy( ( pScratchPad->m_vPlayerOrigin), pStructMem->m_vecPrevPosition );	
	}
	...
	if( flDeltaTime > 0.0 )
	{
		...
		pStructMem->m_flOutPosition[0] = ( pScratchPad->m_vPlayerOrigin )[0];
		pStructMem->m_flOutPosition[1] = ( pScratchPad->m_vPlayerOrigin )[1];
		pStructMem->m_flOutPosition[2] = ( pScratchPad->m_vPlayerOrigin )[2];

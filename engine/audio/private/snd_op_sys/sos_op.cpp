...
void CSosOperator::PrintIO( CSosOperator_t *pStructMem, CSosOperatorStack *pStack, int nLevel ) const
{
	...
	for(int i = 0; i < nNumberOfInputs; i++ )
	{
		...
		DMsg( "SndOperators", 1, "%*s%s: ", nLevel, "    ", pInputName );
		...
		for( int j = 0; j < inputData.m_nCount; j++ )
		{
			...
			{
			case SO_FLOAT: 
				DMsg( "SndOperators", 1, "%f ", *((float*) pInputValue + j) );
				break;
			default:
				DMsg( "SndOperators", 1, " UNKNOWN DATA TYPE %i ", inputData.m_Type );
				break;
			}
		}
		if( bAnyConnected )
		{
			...
			DMsg( "SndOperators", 1, "%%(connected: %s", pStack->GetOperatorName( nOpIndex ) );

			if( nNumConnections > 0 )
			{
				for( int k = 0; k < nNumConnections; k++ )
				{
					DMsg( "SndOperators", 1, " %i", nConnections[k] );
				}
			}
			DMsg( "SndOperators", 1, ")" );
		}
	...
	for(int i = 0; i < nNumberOfOutputs; i++ )
	{
		...
		DMsg( "SndOperators", 1, "%*s%s: ", nLevel, "    ", pOutputName );
		...
		for( int j = 0; j < outputData.m_nCount; j++ )
		{
			...
			{
			case SO_FLOAT:
				DMsg( "SndOperators", 1, "%f ", *((float*) pOutput + j) );
				break;
			default:
				DMsg( "SndOperators", 1, " UNKNOWN DATA TYPE %i ", outputData.m_Type );
				break;
			}
		}
		DMsg( "SndOperators", 1, "\n" );
...
bool CSosOperator::BaseParseKV( CSosOperatorStack *pStack, CSosOperator_t *pStructMem, const char *pParamString, const char *pValueString ) const
{
	...
	if ( pParamString && *pParamString && pValueString && *pValueString )
	{
		...
		{
			// DWarning( "SndOperators", 1, "Error: Unable to find input parameter %s", pParamString );
			return false;
		}
		...
		if ( nOutputOffset > -1 )
		{
			if ( nInputIndex > -1 )
			{
				...
				{
					DMsg( "SndOperators", 1, "Error: Operator %s, illegal array access: %s", pStack->m_pCurrentOperatorName, pParamString );
				}
				return true;
			}
			else
...
int CSosOperator::GetOutputOffset( const char *pOutputName ) const
{
	...
	{
		DMsg( "SndOperators", 1, "Error: Unable to find referenced sound operator output: %s", pOutputName );
		return -1;
	}
...
void CSosOperator::StackShutdown( void *pVoidMem, CSosOperatorStack *pStack, int nOpIndex )
{
	...
	{
		DMsg( "SndOperators", 1, "Stack Shutdown: Stack: %s : Operator: %s\n", pStack->GetName(), pStack->GetOperatorName( nOpIndex ) );
	}
}

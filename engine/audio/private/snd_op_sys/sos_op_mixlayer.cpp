...
void CSosOperatorMixLayer::Execute( void *pVoidMem, channel_t *pChannel, CScratchPad *pScratchPad, CSosOperatorStack *pStack, int nOpIndex ) const
{
	...
	{
		DWarning( "SndOperators", 1, "Error: MixLayer operator has invalid mix group index!\n" );
		return;
	}
	if( pStructMem->m_nMixLayerIndex < 0 )
	{
		DWarning( "SndOperators", 1, "Error: MixLayer operator has invalid mix layer index!\n" );
		return;
	}
		...
		if ( pParamString && *pParamString )
		{
			if ( pValueString && *pValueString )
			{
				...
				else if ( !V_strcasecmp( pParamString, "mixlayer" ) )
				{
					...
					if ( nMixLayerID == -1 )
					{
						DWarning( "SndOperators", 1, "Error: Failed to get mix layer %s!\n", pValueString );
					}
				...
				else if ( !V_strcasecmp( pParamString, "mixgroup" ) )
				{
					...
					if ( nMixGroupID == -1 )
					{
						DWarning( "SndOperators", 1, "Error: Failed to get mix group %s!\n", pValueString );
					}

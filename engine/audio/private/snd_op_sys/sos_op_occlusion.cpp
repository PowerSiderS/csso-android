...
float S_CalcOcclusion( channel_t *ch, const Vector &vecListenerOrigin, Vector vSoundSource, float flOccludedDBLoss );
...
// 		Log_Msg( LOG_SND_OPERATORS, "UPDATING: Sound operator %s\n", pStack->GetOperatorName( nOpIndex ));
		...
		float flGain = S_CalcOcclusion( pChannel,
		...
		flGain = SND_FadeToNewGain( &(pChannel->gain), pChannel, flGain );
	...
	{
		float flGain = SND_FadeToNewGain(  &(pChannel->gain), pChannel, -1.0 );
		flResult = MAX( flResult, flGain );
	}
...
float S_CalcOcclusion( channel_t *ch, const Vector &vecListenerOrigin, Vector vSoundSource, float flOccludedDBLoss )
{
	...
	Ray_t ray;
	ray.Init( MainViewOrigin(), vSoundSource );
	...
	if (tr.DidHit() && tr.fraction < 0.99)
	{
		...
		for (count = 0, i = 0; i < 4; i++)
		{
			...
			Ray_t ray;
			ray.Init( MainViewOrigin(), vSoundSources[i] );
			g_pEngineTraceClient->TraceRay( ray, MASK_BLOCK_AUDIO, &filter, &tr );

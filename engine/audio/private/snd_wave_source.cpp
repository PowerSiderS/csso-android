...
static ConVar snd_prefetch_common( "snd_prefetch_common", "1", FCVAR_NONE, "Prefetch common sounds from directories specified in " SOUND_PREFETCH_FILE );
...
bool CAudioSourceWave::GetXboxAudioStartupData()
{
	...
	Q_snprintf( fileName, sizeof( fileName ), "sound\\%s.wav", tempFileName );
	...
	if ( IsGameConsole() )
	{
		if ( m_hStream == INVALID_STREAM_HANDLE )
		{
			...
			case WAVE_FORMAT_TEMP:
				...
				V_SetExtension( szFilename, ".wav", sizeof( szFilename ) );
...
CAudioMixer *CAudioSourceStreamWave::CreateMixer( int initialStreamPosition, int skipInitialSamples, bool bUpdateDelayForChoreo, SoundError &soundError, hrtf_info_t* pHRTFVec )
{
	...
	if ( IsGameConsole() )
	{
		...
		case WAVE_FORMAT_MP3:
		case WAVE_FORMAT_TEMP:
			V_SetExtension( fileName, ".wav", sizeof( fileName ) );
			break;
...
private:
	...
	bool LoadMasterCache( bool bAllowEmpty );

	void RecursiveBuildSoundList( CUtlRBTree< FileNameHandle_t, int >& other, const char *pCurrentDir, const char *pPathID );
...
bool CAudioSourceCache::LoadMasterCache( bool bAllowEmpty )
{
	m_szMasterCache = CFmtStr( "%s/%s.cache", AUDIOSOURCE_CACHE_ROOTDIR, MASTER_CACHE_NAME );
...
bool CAudioSourceCache::Init( unsigned int memSize )
{
	...
	bool bSuccess = LoadMasterCache( false );
	if ( !bSuccess )
	{
		Warning( "  .cache load failed, forcing rebuild!\n" );
...
bool CAudioSourceCache::UpdateMasterPrecachedSoundsCache()
{
	...
	// Build recursive list of all wav files
	RecursiveBuildSoundList( soundsOnDisk, "sound", "GAME" );
...
bool CAudioSourceCache::BuildMasterPrecachedSoundsCache()
{
	...
	char fn[ 512 ];
	Q_snprintf( fn, sizeof( fn ), "%s/%s.cache", AUDIOSOURCE_CACHE_ROOTDIR, MASTER_CACHE_NAME );
	...
	// Build recursive list of all wav files
	RecursiveBuildSoundList( other, "sound", "GAME" );
...
//-----------------------------------------------------------------------------
void CAudioSourceCache::RecursiveBuildSoundList( CUtlRBTree< FileNameHandle_t, int >& root, const char *pCurrentDir, const char *pathID )
{
	FileFindHandle_t fh;
	FileNameHandle_t handle;
		...
		do
		{
			if ( *fn != '.' )
			{
				if ( g_pFullFileSystem->FindIsDirectory( fh ) )
				{
					...
					RecursiveBuildSoundList( root, nextdir, pathID );
				}
				else
				{
					char ext[ 10 ];
...
CON_COMMAND( snd_rebuildaudiocache, "rebuild audio cache\n" )
{

...
private:
	...
	// PiMoN: bumped from 2^11 (2048) to 16384 because
	// otherwise sound cache will fail to build on
	// first startup!

	// 16384 simultaneous sound handles in the cache
	CUtlHandleTable< WaveCache_t, 14 > m_HandleTable;
				...
				if ( nBytesFilled == 0 )
				{
					...
					if ( snd_async_stream_recover_from_exhausted_stream.GetBool() && ( m_source.Format() == WAVE_FORMAT_PCM ) )
					{
						if ( m_sampleSize <= SIZE_LAST_SAMPLE )
						{
							...
							int nSamplesToFill = min( m_bufferCount, AUDIOSOURCE_COPYBUF_SIZE  / m_sampleSize );
							nSamplesToFill = min( nSamplesToFill, sampleCount );
							if ( copyBuf != NULL )
							{
...
namespace
{
	...
	{
		tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "ProcessPhononJob %d", PHONON_FRAME_SIZE );
		if (!job.phononEffect)
		{
			tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "iplCreateBinauralEffect" );
...
//		if ( !PHONON_USE_SIMD )
		{
			tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "ConvertMultiToFloat" );
...
//		else
		...
		{
			tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "iplApplyBinauralEffect" );
		...
		//if ( !PHONON_USE_SIMD )
		{
			tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "Convert" );

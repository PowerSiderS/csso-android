...
float SND_GetGainObscured( gain_t *gs, const channel_t *ch, const Vector &vecListenerOrigin, bool fplayersound, bool flooping, bool bAttenuated, bool bOkayToTrace, Vector *pOrigin );
...
#if defined(USE_VALVE_HRTF)
static ConVar snd_hrtf_distance_behind("snd_hrtf_distance_behind", "100", FCVAR_DEVELOPMENTONLY, "HRTF calculations will calculate the player as being this far behind the camera\n");
#endif
...
// PiMoN: used in sound operators of current CS:GO build
ConVar weapon_sound_falloff_multiplier( "weapon_sound_falloff_multiplier", "1.0", FCVAR_CHEAT | FCVAR_REPLICATED, "Scaling for falloff of weapon firing sounds" );
...
ConVar snd_musicvolume( "snd_musicvolume", "0.7", FCVAR_ARCHIVE, "Overall music volume", true, 0.0f, true, 1.0f );
ConVar snd_menumusic_volume( "snd_menumusic_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of the main menu music." );
ConVar snd_roundstart_volume( "snd_roundstart_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of round start music." );
ConVar snd_roundend_volume( "snd_roundend_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of round end music." );
ConVar snd_mapobjective_volume( "snd_mapobjective_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of map objective music." );
ConVar snd_tensecondwarning_volume( "snd_tensecondwarning_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of ten second warning music." );
ConVar snd_deathcamera_volume("snd_deathcamera_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of the death camera music.");
ConVar snd_mvp_volume("snd_mvp_volume", "1.0", FCVAR_ARCHIVE, "Relative volume of the MVP music.");
...
static ConVar windows_speaker_config("windows_speaker_config", "-1", FCVAR_CHEAT|FCVAR_ARCHIVE);
...
{
	return check.DistToSqr( listener_origin );
}
/*
=================
...
float SND_GetGain( gain_t *gs, const channel_t *ch, const Vector &vecListenerOrigin, bool fplayersound, bool fmusicsound, bool flooping, vec_t dist, bool bAttenuated, bool bOkayToTrace )
{
	...
	else
	{
		...
		{
			gain = gain * SND_GetGainObscured( gs, ch, vecListenerOrigin, fplayersound, flooping, bAttenuated, bOkayToTrace, NULL );
		}
...
{
	return (listener_origin != vec3_origin &&
...
void SND_SetSpatialDelays()
{
	...
	Vector vecListenerRight = listener_right;
	...
	if ( tr.DidHit() )
	{
		checkDist = VectorLength( tr.endpos - MainViewOrigin() );	
	}
...
void DAS_StoreNode( das_room_t *proom, int dsp_preset)
{
	...
	if ( !proom->bskyabove )
	{
		...
		g_das_nodes[i].range_max = MIN(DAS_DIST_MAX, MIN(proom->width_max * 5, proom->length_max) ); 
...
void SND_ExecuteUpdateOperators( channel_t *ch )
{
	...
	{
		DWarning( "SoundOperatorSystem", 1, "Voice channel attempting to be processed by operators" );
		// Voice_Spatialize( ch );
	}
#endif
...
void SND_Spatialize(channel_t *ch)
{
	...
	if (ch->wavtype == CHAR_HRTF)
	{
		...
		if (ch->hrtf.debug_lock_position == false)
		{
			...
			Vector listener_origin_modified = listener_origin - distance_behind*listener_forward;
			const float dist_to_sound = MIN((listener_origin_modified - origin).Length(), (listener_origin - origin).Length());
			if ( dist_to_sound < distance_behind )
			{
				listener_origin_modified = listener_origin - dist_to_sound*listener_forward;
			}

			// sound_pos is really sound_pos_listener_relative
			Vector sound_pos = listener_origin_modified - origin;
			VectorAngles(listener_forward, listener_angles);
		...
		Vector diff = listener_origin - origin;
	...
	if ( fdopplerwav )
	{
		...
		// point of closest approach to listener, 
		// along sound source forward direction (doppler wavs)
		Vector nearestPoint = ch->origin; // default nearest sound approach point

		// calculate point of closest approach for CHAR_DOPPLER wavs, replace source_vec

		fplaydopplerwav = SND_GetClosestPoint( ch, listener_origin, source_angles, nearestPoint );
		...
		SND_GetDopplerPoints( ch, listener_origin, source_angles, nearestPoint, source_doppler_left, source_doppler_right);
	...
	if ( !fplayersound )
	{
		ch->dspface = SND_GetFacingDirection( ch, listener_origin, source_angles );
	...
	gain = SND_GetGain( &ch->gain, ch, listener_origin, fplayersound, fmusicsound, looping, dist, bAttenuated, bOkayToTrace );
...
//	gain *= MXR_GetVolFromMixGroup( ch, &last_mixgroupid );
	...
	if ( fdopplerwav )
	{
		...
		Device_SpatializeChannel( volumes, ch->master_vol, source_vec_DL, gain, mono, int(ch->wavtype) );
		...
		Device_SpatializeChannel( volumes, ch->master_vol, source_vec_DR, gain, mono, int(ch->wavtype) );
		// load volumes into channel as crossfade targets
	...
	else
	{
		...
		float volumes[CCHANVOLUMES/2] = { 0 };
		Device_SpatializeChannel( volumes, ch->master_vol, source_vec, gain, mono, int(ch->wavtype) );
...
void SND_SpatializeFirstFrameNoTrace( channel_t *pChannel)
{
		...
		!toolframework->InToolMode() )
	{
		...
		pChannel->gain.ob_gain = 1.0;
		pChannel->gain.ob_gain_inc = 1.0;
		pChannel->gain.ob_gain_target = 1.0;
		...
		pChannel->gain.ob_gain = 0.0;
		pChannel->gain.ob_gain_inc = 0.0;
		pChannel->gain.ob_gain_target = 0.0;
...
// 			pStackList->SetScriptHash( params.m_nSoundScriptHash );
	...
	if ( params.delay != 0.0f )
	{
		...
		else
		{
			...
			ch->gain.ob_gain_target	= 1.0f;
			ch->gain.ob_gain		= 1.0f;
			ch->gain.ob_gain_inc	= 0.0f;
			ch->flags.bfirstpass = false;
			ch->flags.delayed_start = true;
		}
	}
...
void S_CompareSoundParams( StartSoundParams_t &pStartParams , CSoundParameters &pScriptParams )
{
	...
	{
		DWarning( "SoundOperatorSystem", 1, "Warning: SoundEntry %s has differing emitter and script channels: emitter %i : script %i\n",
	...
	{
		DWarning( "SoundOperatorSystem", 1, "Warning: SoundEntry %s has differing emitter and script delay times: emitter %f : script %d\n",
										pStartParams.m_pSoundEntryName,
										pStartParams.delay,
...
// 	}
	...
	//	V_strncpy( sosEntryMatch.m_nMatchString2, pScriptParams.soundname, sizeof( sosEntryMatch.m_nMatchString2 ) );
		...
		if( g_pSoundOperatorSystem->m_sosEntryBlockList.HasAMatch( &sosEntryMatch ) )
		{
			...
			{
				DMsg( "SoundOperatorSystem", 1, "Entry Blocked: %s\n", pStartParams.m_pSoundEntryName );
			}
			return 0;
		}
...
void S_Update( const AudioState_t *pAudioState )
{
	...
	VectorCopy( listener_origin, g_scratchpad.m_vPlayerOrigin );
	VectorCopy( listener_forward, g_scratchpad.m_vPlayerForward );
	VectorCopy( listener_right, g_scratchpad.m_vPlayerRight );
	VectorCopy( listener_up, g_scratchpad.m_vPlayerUp );
